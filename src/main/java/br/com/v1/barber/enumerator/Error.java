package br.com.v1.barber.enumerator;

import lombok.AllArgsConstructor;
import lombok.Getter;

/**
 * Enum representing different error types and their messages.
 */
@Getter
@AllArgsConstructor
public enum Error {

    NO_CLIENT_FOUND("NO_CLIENT_FOUND", "No CLIENT were found."),

    NO_CLIENT_FOUND_BY_ID("NO_CLIENT_FOUND_BY_ID", "No CLIENT was found by ID."),
    NO_CLIENT_FOUND_BY_FIRST_AND_LAST_NAME("NO_CLIENT_FOUND_BY_FIRST_AND_LAST_NAME",
            "No CLIENT was found by first and last name."),
    CLIENT_ALREADY_EXISTS("CLIENT_ALREADY_EXISTS", "CLIENT already exists."),
    CLIENT_ALREADY_REGISTERED("CLIENT_ALREADY_REGISTERED", "Client is already registered."),

    NO_SERVICE_FOUND("NO_SERVICE_FOUND", "No SERVICE were found."),
    NO_SERVICE_FOUND_BY_ID("NO_SERVICE_FOUND_BY_ID", "No SERVICE was found by ID."),
    SERVICE_ALREADY_EXISTS("SERVICE_ALREADY_EXISTS", "SERVICE already exists."),
    SERVICE_ALREADY_REGISTERED("SERVICE_ALREADY_REGISTERED", "SERVICE already registered."),

    NO_EMPLOYEE_FOUND("NO_EMPLOYEE_FOUND", "No EMPLOYEE were found."),
    NO_EMPLOYEE_FOUND_BY_ID("NO_EMPLOYEE_FOUND_BY_ID", "No EMPLOYEE was found by ID."),
    EMPLOYEE_ALREADY_EXISTS("EMPLOYEE_ALREADY_EXISTS", "EMPLOYEE already registered."),
    EMPLOYEE_ALREADY_REGISTERED("EMPLOYEE_ALREADY_REGISTERED", "EMPLOYEE already registered."),

    NO_APPOINTMENT_FOUND("NO_APPOINTMENT_FOUND", "No APPOINTMENT were found."),
    NO_APPOINTMENT_FOUND_BY_ID("NO_APPOINTMENT_FOUND_BY_ID", "No APPOINTMENT was found by ID."),
    APPOINTMENT_ALREADY_EXISTS("APPOINTMENT_ALREADY_EXISTS", "APPOINTMENT already registered."),
    APPOINTMENT_ALREADY_REGISTERED("APPOINTMENT_ALREADY_REGISTERED", "APPOINTMENT already registered."),
    APPOINTMENT_NOT_REGISTERED("APPOINTMENT_NOT_REGISTERED", "APPOINTMENT not registered."),

    TIME_NOT_AVAILABLE("TIME_NOT_AVAILABLE", "Time not available. ");


    private final String errorMessage;
    private final String errorDescription;
}
